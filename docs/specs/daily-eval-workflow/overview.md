...
All default LLM calls use Chat Completions (JSON mode when `GREENNODE_JSON_MODE=true`). If `MODEL_JUDGE` is set to a GPT-5 model, that call is automatically routed through the **Responses API** (non-streaming, `reasoning: medium`). Model swap requires only `MODEL_*` env var changes — no code changes.
